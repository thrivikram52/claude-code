...
This repository already includes ready-to-use Claude Code configuration and context files:

- **Claude Code config (`.claude/`)**
  - `agents/`: prebuilt subagents (e.g., `Architect.md`, `QA.md`, `Security.md`)
  - `commands/`: custom slash commands (e.g., `primer.md`, `generate-prp.md`, `fix-github-issue.md`, `WorkTrees.md`)
  - `hooks/`: example hooks (`log-tool-usage.sh`, `example-hook-config.json`)
  - `settings.local.json`: sample tool allowlist and hook wiring
- **Context files**
  - `CLAUDE.md`: comprehensive project guidelines for Claude Code
  - `VibeCode.md`: workflow tips and practices
- **UI Design (`.superdesign/`)**
  - `design_iterations/`: UI design specifications (e.g., `calculator-ui-v1.md`)
  - `superdesign.config.json`: design system configuration
  - `README.md`: Superdesign usage guide

## 🏁 Quick start for this repo

1. Install Claude Code (see Installation below)
2. Open this repository in your terminal and start Claude Code:
   ```bash
   claude
   ```
3. Prime the repo context and review permissions:
   - Run `/primer` to analyze the codebase and load context
   - Run `/permissions` to review or import the allowlist from `.claude/settings.local.json`
4. Optional: enable the example hook script
   ```bash
   chmod +x .claude/hooks/log-tool-usage.sh
   ```

### 🎨 Quick start with Superdesign (UI Design)

1. Install Superdesign CLI:
   ```bash
...
2. View available designs:
   ...
   superdesign list
   ```
3. Generate UI components from a design:
   - **With Claude Code**: Run `/design calculator-ui-v1`
   - **With Cursor/Windsurf**: Use `@superdesign design calculator-ui-v1`
   - **With CLI**: Run `superdesign generate calculator-ui-v1`
...
7. **Design UI Components**: Use Superdesign for calculator and other UI designs
